...
/*   Updated: 2018/02/01 18:14:37 by imelnych         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		print_digits_unsigned(va_list *args, list_spec *cr, int type)
{
	...
	if (!(ft_strcmp(cr->str, s)) && cr->precs >= 0 && (type == 'x' || type == 'X'))
	{
		cr->str = "";
		while (cr->width > (int)ft_strlen(cr->str))
			cr->str = ft_strjoin_free(" ", cr->str, 0); //fail if 2, as first time I try to free "" empty cr->str
	}
	else if (!(ft_strcmp(cr->str, s)) && cr->precs >= 0 && !cr->flag[2] && (type == 'o' || type == 'O'))
	{
		cr->str = "";
		while (cr->width > (int)ft_strlen(cr->str))
			cr->str = ft_strjoin_free(" ", cr->str, 0); //fail if 2, as first time I try to free "" empty cr->str
	...
	{
		if (!(ft_strcmp(cr->str, s)) && cr->flag[2])
			cr->flag[2] = 0;
